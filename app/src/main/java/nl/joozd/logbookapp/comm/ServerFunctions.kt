...
object ServerFunctions {
    ...
    fun getAirports(client: Client, listener: (Int) -> Unit): List<BasicAirport>?{
        ...
        return client.readFromServer(listener)?.let {
            unpackSerialized(it).map { a -> BasicAirport.deserialize(a)}
        }
    }
    ...
    /**
     * Checks a list of flights against server to see if their IDs are already known. If so, it fixes those numbers.
     * @param client An initialized Client to send/receive data
     * @param flightsToCheck a List of Flights to check IDs for. Don't use on flights already synched.
     * @return the checked flights with server-OK numbers
     */
    fun checkIDs(client: Client, flightsToCheck: List<Flight>): List<Flight>{
        client.sendRequest(JoozdlogCommsKeywords.CHECK_NEW_ID_NUMBERS,flightsToCheck.map{it.flightID.toByteArray().toList()}.flatten().toByteArray())
        val takenNumbers = client.readFromServer()?.let{
            it.toList().chunked(4).map{intAsBytes ->
                intFromBytes(
                    intAsBytes
                )
            }
        } ?: error ("Failed to read IDs from server")
        if (takenNumbers.isEmpty()) return flightsToCheck
        val highestTaken = takenNumbers.last()
        var counter = 0
        return flightsToCheck.map {
            if (it.flightID !in takenNumbers) it else {
                counter++
                it.copy(flightID = highestTaken + counter)
            }
        }
    }
