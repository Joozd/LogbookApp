...
object MessagesWaiting: JoozdLogPreferences() {
    ...
    val noDialog by JoozdlogSharedPreferenceDelegate(NO_DIALOG_MESSAGE_SCHEDULED, false)
    val noMessageBar by JoozdlogSharedPreferenceDelegate(NO_MESSAGEBAR_MESSAGE_SCHEDULED, false)
    ...
    private val noDialogMessageFlow = noDialog.flow.map { if (it) PersistentDialogMessage.NO_DIALOG else null}
    ...
    fun getDialogMessageToDisplayFlow(): Flow<PersistentDialogMessage?> = combine(
        ...
        messagesWaiting.filterNotNull().first()
    }
    ...
    private val noMessageBarMessageFlow = noMessageBar.flow.map { if (it) MessageBarMessage.NO_MESSAGE else null}
