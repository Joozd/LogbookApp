...
class WorkingFlightOld private constructor(flight: Flight): CoroutineScope by dispatchersProviderMainScope() {
    ...
    val crewLiveData = _wfLiveData.map { AugmentedCrew.of(it.augmentedCrew) }
    val isAugmented = _wfLiveData.map { AugmentedCrew.of(it.augmentedCrew).size > 2}
    ...
    var crew: AugmentedCrew
        get() = AugmentedCrew.of(wf.augmentedCrew)
        set(it) {
    ...
    companion object {
         ...
         * @return a [WorkingFlightOld] object of this flight and runs all updates on it.
         */
        suspend fun fromFlightID(id: Int) = withContext(Dispatchers.Main){
            FlightRepositoryImpl.getInstance().fetchFlightByID(id)?.let { WorkingFlightOld(it) }
        }
         ...
         */
        suspend fun createNew(): WorkingFlightOld {
            ...
            return withContext(Dispatchers.Main){WorkingFlightOld(f)
            }
        }
    }
